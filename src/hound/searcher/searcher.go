...
func NewFromExisting(vcsDir string, repo *config.Repo) (*Searcher, error) {
	...
	sha, err := vcs.HeadHash(repo.VCS, vcsDir, repo.Branch)
	if err != nil {
		return nil, err
	}
...
func New(vcsDir string, repo *config.Repo) (*Searcher, error) {
	...
	sha, err := vcs.PullOrClone(repo.VCS, vcsDir, repo.Url, repo.Branch)
	if err != nil {
		return nil, err
	}
	...
	go func() {
		for {
			...
			newSha, err := vcs.PullOrClone(repo.VCS, vcsDir, repo.Url, repo.Branch)
			if err != nil {
